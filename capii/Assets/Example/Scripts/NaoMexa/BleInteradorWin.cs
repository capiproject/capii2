...
public class BleInteradorWin :  IConexao
{
    ...
    //Aqui no construtor vc deve colocar os objetos que estão na interface que
    //você deseja usar
    ...
    async void Subscreve()
    {
        status.text = "Subescrevendo...1";
        BleApi.ScanServices(_deviceUuid);
        await Task.Delay(3000);
        status.text = "Subescrevendo...2";
        BleApi.ScanCharacteristics(_deviceUuid, _servico);
        await Task.Delay(3000);
        status.text = "Pronto";
        BleApi.SubscribeCharacteristic(_deviceUuid, _servico, _caracteristica, false);
    ...
    //Aqui é que as coisas podem ser alteradas
